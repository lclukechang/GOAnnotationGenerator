...
# Handles output from GeneMANIA command line tool: GeneSanitizer. Splits sanitized list into 2 lists (query, missing)
...
# Sanitizes and handles sanitized output by splitting into genes recognized and unrecognized by GeneMANIA
def sanitize_split(unsanitized):
	str_input = '\n'.join(unsanitized)
	sanitizer_ret = settings.javacmd('org.genemania.plugin.apps.GeneSanitizer', 
        '--data', settings.data, '--organism', "H. Sapiens", _in=str_input)
